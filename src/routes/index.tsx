...
import { ADDRESS_MAP_URL, instagramHref, normalizeAddress, normalizeInstagram, normalizePhone, phoneHref } from "@/lib/contact";
...
const HERO_FALLBACK = "https://images.unsplash.com/photo-1621605815971-fbc98d665033?auto=format&fit=crop&w=1800&q=85";
...
function Landing() {
  ...
  const displayAddress = normalizeAddress(settings?.address);
  const displayInstagram = normalizeInstagram(settings?.instagram);
  ...
  return (
    <div className="min-h-screen bg-white text-[#171411]">
      ...
      <Section label="Отзывы" title="Клиенты возвращаются" subtitle={`Коротко о том, почему выбирают ${shopName}.`}>
        <div className="grid gap-4 md:grid-cols-3">
          {[
            ...
            <div key={r.name} className="border border-[#171411]/12 bg-[#f8f8f8] p-7 transition-all duration-300 hover:-translate-y-1 hover:bg-white hover:shadow-sm">
      ...
      <section id="contacts" className="px-5 py-20 md:py-28">
        <div className="mx-auto max-w-7xl border border-[#171411]/14 bg-[#f8f8f8] p-7 shadow-sm md:p-12">
          <div className="grid gap-10 md:grid-cols-2 md:items-end">
            <div>
              ...
              <div className="mt-8 grid gap-4 text-sm sm:grid-cols-2">
                <ContactItem icon={<MapPin className="h-4 w-4" />} label="АДРЕС" value={displayAddress} href={ADDRESS_MAP_URL} external />
                <ContactItem icon={<Phone className="h-4 w-4" />} label="ТЕЛЕФОН" value={displayPhone} href={phoneHref(displayPhone)} />
                <Info label="ЧАСЫ РАБОТЫ" value={normalizeWorkingHours(settings?.working_hours)} />
                <ContactItem icon={<Instagram className="h-4 w-4" />} label="INSTAGRAM" value={displayInstagram} href={instagramHref(displayInstagram)} external />
              </div>
            </div>
      ...
      <SiteFooter shopName={shopName} phone={displayPhone} address={displayAddress} instagram={displayInstagram} />
    </div>
  );
}
...
function ServiceCard({ s, index }: { s: Service; index: number }) {
  return (
    <Link to="/booking" search={{ service: s.id }} className="group flex min-h-36 flex-col justify-between border border-[#171411]/12 bg-[#f8f8f8] p-4 transition-all duration-300 hover:-translate-y-1 hover:border-[#171411]/35 hover:bg-white hover:shadow-sm" style={{ transitionDelay: `${Math.min(index * 35, 160)}ms` }}>
      <div>
