...
function BookingsAdmin() {
  ...
  async function load() {
    setLoading(true);
    let q = supabase.from("bookings").select("*, services(name), masters(name)").order("booking_date", { ascending: false }).order("booking_time", { ascending: false });
  ...
  return (
    <div>
      ...
      <div className="mt-10 border border-divider">
        <div className="grid grid-cols-[100px_100px_1fr_140px_1fr_1fr_120px_160px] gap-4 px-4 py-3 border-b border-divider font-display text-[10px] tracking-[0.25em] text-muted-foreground bg-card">
        ...
        {rows.map((r) => (
          <div key={r.id}
            className={`grid grid-cols-[100px_100px_1fr_140px_1fr_1fr_120px_160px] gap-4 px-4 py-4 border-b border-divider items-center text-sm ${r.status === "new" ? "border-l-2 border-l-gold" : ""}`}>
            ...
            <div className="flex justify-end gap-2">
              {r.status !== "confirmed" && (
                <button onClick={() => updateStatus(r.id, "confirmed")} className="px-3 py-1.5 border border-gold text-gold text-xs hover:bg-gold hover:text-black transition-colors">✓</button>
              )}
              {r.status !== "cancelled" && (
                <button onClick={() => updateStatus(r.id, "cancelled")} className="px-3 py-1.5 border border-divider text-muted-foreground text-xs hover:border-destructive hover:text-destructive transition-colors">✕</button>
              )}
            </div>
          </div>
        ))}
